...
const ERC1155MixedFungibleMintable = artifacts.require('ERC1155MixedFungibleMintable.sol');
...
contract('ERC1155MixedFungibleMintable - tests all core 1155 functionality.', (accounts) => {
    before(async () => {
        user1 = accounts[1];
        user2 = accounts[2];
        user3 = accounts[3];
        user4 = accounts[4];
        mainContract = await ERC1155MixedFungibleMintable.new();
    ...
    it('Create initial items', async () => {
        ...
        function verifyMintTransfer(tx, value, creator) {
            for (let l of tx.logs) {
                if (l.event === 'TransferSingle') {
                    assert(l.args._operator === creator);
                    // This signifies minting.
        ...
        tx = await mainContract.create(hammerUri, false, {from: user1});
        hammerId = verifyCreateTransfer(tx, user1);
        ...
        tx = await mainContract.mintFungible(hammerId, [user1], [hammerQuantity], {from: user1});
        verifyMintTransfer(tx, hammerQuantity, user1);


        // let swordQuantity = 200;
        // let swordUri = 'https://metadata.enjincoin.io/sword.json';
        // tx = await mainContract.create(swordUri, true, {from: user1});
        ...
        // tx = await mainContract.create( maceUri, true, {from: user1});
